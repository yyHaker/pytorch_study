...
class Translator(object):
    ...
    def __init__(self, checkpoint=None,
        ...
        :param target_tok:
        :param beam_size:
        :param length_normalization_factor:
    ...
    def translate(self, input_sentences, target_priming=None):
        """
        :param input_sentences: either a string or list of strings
        :param target_priming:
        :return:
        """
        if isinstance(input_sentences, list):
            ...
            flatten = True
        batch = len(input_sentences)  # batch_size 句子的个数？
        # 对input_sentences每一个句子进行分词，并在开始和结尾插入相应的符号
        src_tok = [self.src_tok.tokenize(sentence,
                                         insert_start=self.insert_src_start,
                                         insert_end=self.insert_src_end)
                   for sentence in input_sentences]
        # 对target_priming每一个句子进行分词，并在开始插入相应的符号
        if target_priming is None:
            bos = [self.insert_target_start] * batch    # [batch]
        else:
        ...
        # Runs beam search sequence generation
        seqs = self.generator.beam_search(bos, state_list)
        # remove forced  tokens
        preds = [s.sentence[len(self.insert_target_start):] for s in seqs]  # 去除开始符号
        output = [self.target_tok.detokenize(p[:-1]) for p in preds]  # 得到的结果
